...
SoundMonitor.Functions = {
    getFieldForTable: function(schemaInstance, fieldName, extra){
        ...
        var label = schemaInstance._schema[fieldName] && (schemaInstance._schema[fieldName].columnHeader
            || schemaInstance._schema[fieldName].label);
    ...
    generateDeleteEventHandlers : function(){
        return {
            'click .delete-confirm' : function () {
                ...
                var col = SoundMonitor.getCollection(Router.current().route.getName());
            ...
            'click .delete-deny' : function () {
                Router.go(Template.soundAlert.previousRoute || Router.current().route.options.parent);
            }
        }
    }
};