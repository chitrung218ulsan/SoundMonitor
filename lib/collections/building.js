...
var buildingSchema = new SimpleSchema([SoundMonitor.BaseSchema, {
	...
	soundThresholdSetingDate:{
		type:Date,
		label: "Soung Seeting Date",
		autoform: {
		  afFormGroup : {
			type: "date"
		  }
...
Building.helpers({
    ...
    homes: function(){
        return Home.find({buildingId: this._id, isDeleted: false},{sort:{floorNumber:1,homeNumber:1}});
    },
	...
	numOFHomesPerFloor:function(floorNumber){
		
		return this.homesWithFloor(floorNumber).count();
	},
    numOfHomes: function(){
        return this.homes().count();
    },
	totalHomesInBuilding:function(){
		return  (this.numOfFloors * this.numHousePerFloor);
	},
